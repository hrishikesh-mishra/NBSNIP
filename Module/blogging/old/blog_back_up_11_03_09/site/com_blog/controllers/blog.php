...
defined('_JEXEC') or die ('Ristricted Access ');
 ...
 class blogController extends JController
 {
	...
	function display($bmsg=null,$act=null)
	{
		...
		if ($act)
		{
			...
			{
				case 1:
						$txtsOnTopic= JRequest::getVar('txtsOnTopic');
						if (empty($txtsOnTopic))
							$bmsg="Your Search Key is empty.<br/>";
						
						$bDData=$model->loadBlogByTopic($txtsOnTopic);
						if (!count($bDData))
						{
								$bmsg="Blog in this topic is not found..<br/>";
						}
						break;
						
				case 2:
						$txtBlogOfBlogger=trim(JRequest::getVar('txtBlogOfBlogger'));
						
						if(empty($txtBlogOfBlogger))
							$bmsg="Your Search Key is empty.<br/>";
						
						$bDData=$model->loadBlogOfBlogger($txtBlogOfBlogger);
						if (!count($bDData))
						{
								$bmsg="Blogs of your search blogger  is not found..<br/>";
						}
						break;
						
				case 3:
						$searchDate =trim(JRequest::getVar('searchDate'));
						
						if(empty($searchDate))
							$bmsg="Search Date is empty.<br/>";
						
						$bDData=$model->loadBlogByDate($searchDate);
						if (!count($bDData))
						{
								$bmsg="Blogs on your search date is not found..<br/>";
						}
						break;
				
				default :
							$bmsg="Unable to process your request.<br/>";
			}
		}
		else
		{
			$bDData=$model->loadBlogByDate();
		}
							
		require_once (JPATH_COMPONENT.DS.'views'.DS.'blog'.DS.'view.html.php');
		$view = new blogViewBlog();	
		$view->assign('msg',$this->mMsg);
		$view->assign('bDData',$bDData);
		$view->assign('bmsg',$bmsg);
		$view->display();
		$view->display('sblog');
		return true;
	}
	...
	function addCmt()
	{
		...
		$blogid=trim(JRequest::getVar('blogid'));
		$cname= trim(JRequest::getVar('cname'));
		$textimg =trim(JRequest::getVar('textimg'));
		$comment=trim(JRequest::getVar('comment'));
		
		$msg='';
		$flag=false;
		
		//print_r(JRequest::getVar('POST'));
		if (empty($blogid))
		{
			$msg .='Unable to Process your request. <br/>';
			$flag=true;
		}
		
		if (empty($cname))
		{
			$msg .='Commenter Name is empty.<br/>';
			$flag=true;
		}
		...
		if (empty($comment))
		{
			$msg .='Comment is emtpy.<br/>';
			$flag=true;
		}
		
		$sess=& JFactory::getSession();
		$cap=$sess->get('captcha1',null,'blog');	
		$sess->set('captcha1',null,'blog');
		if (strtolower($cap) !=strtolower($textimg))
		{
		...
		if($model->SaveEditCmt($data))
		{
			$this->minf('Your Comment has been saved.');
			return true;
		}
		else
		{
			$this->minf('Sorry,Unable to save your comment.');
			return false;
		}
	}	
	
	function searchOnTopic()
	{
		$this->display('',1);
		return true;
	}	
	
	function searchBloger()
	{
		$this->display('',2);
		return true;
	}
