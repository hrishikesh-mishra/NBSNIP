 ...
 */
 ...
 class yellowpageControllerEducation extends JController
 {
	 ...
	 function publish()
	{
		...
		$model = $this->getModel('Education');
		if ($model->changeEducation($cid,1))
		{
		$message =JText::_('Education Information Published '); 
		}
		else
		{
			$message= JText::_('Education publish Failed');
			$message .= ' ['. $model->getError() .']';
		}
		$this->setRedirect('index.php?option=com_yellowpage&controller=education', $message );
	}
	...
	function unpublish()
	{
		...
		$model = $this->getModel('Education');
		if ($model->changeEducation($cid,0))
		{
		$message =JText::_('Education unPublished'); 
		}
		else
		{
			$message= JText::_('Education publish Failed');
	...
	function remove()
	{
		...
		$this->setRedirect( 'index.php?option=com_yellowpage&controller=education', $msg );
	}
