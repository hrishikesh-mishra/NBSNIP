<?php 
	...
	*/
	...
	require_once (JPATH_COMPONENT_ADMINISTRATOR.DS.'tables'.DS.'hotellodge.php');
	...
	function &getHotelLodge()
	{
		...
		{
			$where[] = 'hotelname LIKE '.$db->Quote( '%'.$db->getEscaped( $this->_searchkey, true ).'%', false );
		...
		if(empty($this->_data))
		{
			$query = "select * from #__yp_hotel_lodge " .$where; 
	...
	function getHotellodgeVal( $id )
	{
		$query	= "select * from #__yp_hotel_lodge where (published > 0) and id =" .$id ;
