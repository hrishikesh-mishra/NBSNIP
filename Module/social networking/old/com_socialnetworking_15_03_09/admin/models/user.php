...
defined('_JEXEC') or die ('Ristricted Access ');
	...
	class socialnetworkingModelUser extends JModel
	{  
	...
	function save ($data)
	{
		...
		if (empty($pwd))
		{
			$query="select password from #__sn_user where id='" .$data['id'] ."'";
	...
	function getTotal()
	{
		...
		{
			$this->_total = $this->_getListCount("select * from #__sn_user");
		}
		return $this->_total;
	}
